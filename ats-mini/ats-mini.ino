...
uint8_t knobMode = KNOB_MODE_VOLUME; // Default: volume
  ...
  else
  {
    ...
    if(encCount)
    {
      switch(currentCmd)
      {
        case CMD_NONE:
        {
          // Knob mode determines action when no modal is active
    ...
    else if(pb1st.wasClicked || pb1st.wasShortPressed)
    {
      ...
      // Toggle knob mode on short press when in idle (CMD_NONE)
      else if(pb1st.wasShortPressed && currentCmd == CMD_NONE)
      {
        // Toggle knob mode
        knobMode = (knobMode == KNOB_MODE_VOLUME) ? KNOB_MODE_FREQUENCY : KNOB_MODE_VOLUME;
        needRedraw = true;
        elapsedSleep = elapsedCommand = millis();
      }
      // Handle other modal button presses
      ...
      // If in a modal and click didn't handle, exit modal
      else if(currentCmd != CMD_NONE)
      {
        currentCmd = CMD_NONE;
        needRedraw = true;
      }
      // Normal click (not short press) in idle mode: open menu
